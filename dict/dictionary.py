    ...
    with open(sys.argv[2]) as dictfile, open(sys.argv[3]) as exfile:
        ...
        for item in csv.reader(exfile):
            try:
                ...
                continue
            examples[sid] = (item[3], item[4])

        tayal = Dictionary()
        tayal.set_title("Atayal ("+sys.argv[1]+") - Chinese")
        tayal.set_full_title("Atayal ("+sys.argv[1]+") to Chinese Dictionary")
        tayal.set_langs("TAY", "ZHT")
